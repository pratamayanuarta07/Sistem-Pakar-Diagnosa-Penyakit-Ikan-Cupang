...
<header>Pilih Gejala yang diderita ikan anda</header>    
    <div class="row">
            <div class="card-body scrollbar">
            <form onsubmit="return validateForm()" action="<?= base_url('FC')?>" method="POST">
           <?php
            for ($i=0; $i < count($Nama); $i++) {  //untuk menampilkan nama gejala di interfes
              echo '<div class="input-group mb-3">
              <div class="input-group-prepend">
                <div class="input-group-text
                ">
                  <input type="checkbox" name="'.($i+1).'" aria-label="Checkbox for following text input">
                </div>
              </div>
              <input type="text" value="'.$Nama[$i].'" class="form-control" aria-label="Text input with checkbox" readonly>
            </div>';
            }
          ?>
          </div>
